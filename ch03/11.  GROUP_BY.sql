...
-- 일반적으로 집계함수와 함께 사용됨
SELECT observe_date FROM jeju2 GROUP BY observe_date; # 6개의 레코드가 나옴! = DISTINCT 
SELECT observe_date, speed_80m FROM jeju2 GROUP BY observe_date; -- 이거안됨
...
-- 주의 WHERE 절과 사용방법이 같지만 WHERE절은 SELECT 이전에 실행되고 
...
FROM jeju2 
GROUP BY observe_date
HAVING speed_80m > 5.5; # speed_80m을 쓰고 싶으면 위에 observe_date, 뒤에 써야함

SELECT observe_date, AVG(speed_80m) AS avg 
FROM jeju2 
WHERE speed_80m > 5.5
GROUP BY observe_date; # WHERE절과  HAVING 절의 차이를 보여줌!
