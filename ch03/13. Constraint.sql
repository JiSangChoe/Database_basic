...
-- NOT NULL 제약조건 : 해당컬럼에 null을 포함하지 못하도록 함
-- INSERT, UPDATE에 영향을 미침
CREATE TABLE not_null_table (
    null_column INT,
    not_null_column INT NOT NULL
);
-- NOT NULL 제약조건이 ㄱ걸린 null_column에 값을 지정하지 않음
INSERT INTO not_null_table (null_column) VALUES(1);

-- NOR NULL이 지정된 컬럼은 INSERT시에 '반드시' null이 아닌 
-- 값이 입력되어야 함
...
-- 자신 테이블에서의 INSERT, UPDATE /
-- 참조되어지는 테이블의 INSERT, UPDATE에 영향을 미침
...
-- 참조하고 잇는 테이블의 컬럼에 값이 존재하지 않으면 지정 불가
...
-- 특정 테이블에서 기본키를 참조하고 있는 레코드가 존재한다면 
-- 해당 레코드를 삭제하지 못함
...
-- 특정 테이블을 참조하고 잇는 테이블이 존재한다면 테이블 구조를 제거할 수 없음
...
-- ON UPDATE : 참조하고 잇는 테이블의 기본키가 변경되었을 때 동작
-- ON DELETE : 참조하고 잇는 테이블의 기본키가 삭제되었을 때 동작
...
CREATE TABLE optional_foregin_table (
	primary_column INT,
    foreign_column INT,
...
INSERT INTO primary_table VALUES (1, 1); -- 부모테이블 데이터 값
INSERT INTO optional_foregin_table VALUES (1, 1);  

UPDATE primary_table SET primary_column = 3
WHERE primary_column = 1;
SELECT * FROM optional_foregin_table;
